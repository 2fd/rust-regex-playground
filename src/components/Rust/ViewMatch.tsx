          ...
          {len > 0 &&
            props.matches.map((match, i) => {
              return (
                <div
                  key={i}
                  className="mb-4 w-full rounded bg-slate-100 px-4 py-2 font-mono text-sm text-gray-600 shadow ring-1 ring-gray-300 dark:bg-gray-600 dark:text-gray-100 dark:ring-gray-700"
                >
                  <pre>
                    <Math match={match} version={props.version} />
                  </pre>
                </div>
              )
            })}
...
export function Math(props: { match: Match; version?: string }) {
  return (
    <Struct
      value={{
        '@name': 'regex::Match',
        ...
        'end()': props.match.end,
        'as_str()': props.match.value,
      }}
      version={props.version ? 'regex/' + props.version : 'regex/latest'}
    />
  )
}
